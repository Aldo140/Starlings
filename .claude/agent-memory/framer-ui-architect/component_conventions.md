...
## Google Apps Script backend rule: doGet() doesn't respect INFO_START_OFFSET on Live_* (added 2026-07-30)

`doGet()` in `docs/backend/Code.gs.js` reads a sheet's *entire* data range (`sheet.getDataRange().getValues()`) with no lower bound, for both `Pending_*` and `Live_*` tabs alike — it's the one function in the whole backend that doesn't cap at `INFO_START_OFFSET` (50). Every other Pending-side function (`doPost`'s row placement, `approvalOnEdit`, `moveAllCheckedRows_`, `approveCheckedRows`, `sweepApproved`) explicitly bounds itself to stay above that row.

**This means: never write an info block, instructions text, or any non-data content into row 50+ of a `Live_*` sheet** (`writeInfoBlock()` from `ApprovalWorkflow.gs.js` specifically) — it will leak straight into the public API response `doGet()` returns, which the live site renders directly. Whether a leaked row actually shows up as garbage on the site depends on that content type's frontend filter in `services/api.ts`: posts/QA/reflections all require a non-empty content field (message/question+answer/resourceId+reflection) and would silently filter a mostly-blank info-block row out, but **resources have no such filter** (`getApprovedResources()`/`normalizeResource()` accepts any row with *any* id) — a leaked row there renders as a real, broken resource card on the live Resources page. `TAB_INFO` in `ApprovalWorkflow.gs.js` is Pending_*-only for exactly this reason — keep it that way.
