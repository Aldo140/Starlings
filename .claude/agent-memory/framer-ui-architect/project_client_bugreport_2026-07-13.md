...
| Reflections ever displayed publicly (incl. photos) | **Not wanted — do not build.** See correction below. | Agnes's actual question was purely about the spreadsheet (where does the photo land, why won't the checkbox approve it) — not about public display. A "What others shared" public reflections feature was built on 2026-07-14 based on a misread of the question, then reverted the same day (commit 7befbf4 → reverted in 71e5bfb) once Aldo clarified. Do not re-propose this without an explicit, unambiguous request — re-reread the actual client email before inferring product scope from a conversation's drift. |
...
## Correction (2026-07-14) — read this before touching reflections again
Agnes's original email question — *"I also added a photo once someone submits a reflection where does it go? I found the section in the excel 'reflections' but not sure what is happening as it says false and no option to approve"* — is entirely about the **Google Sheet**: (a) where does a submitted photo land as a column/value in `Pending_Reflections`, and (b) why does the Approve checkbox not work. It is NOT a request for reflections to be shown on the public website. Issue 3's fix above (the `image_url` column + Approve checkbox fix) is the complete, correct, and sufficient answer to her actual question — nothing more is needed or wanted. See [[feedback_scope_from_client_questions]] for the general lesson.
